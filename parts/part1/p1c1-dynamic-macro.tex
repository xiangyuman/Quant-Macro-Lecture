...
动态规划是量化宏观经济学的基础。本章首先通过最优增长模型——现代宏观经济学的基石模型，
将序列问题分解为递归为题，进而介绍确定性动态规划的基本理论方法。
接着分析模型的动态特征，引入与其相关的均衡概念，并介绍这一模型的初步数值解法，为后续不确定性动态规划的学习奠定基础。


% ===== 1.1 确定性动态规划 =====
\section{最优增长模型设定} \label{sec-ngm}

现代增长理论始于Ramsey（1928）的经典论文，此后Solow（1956）和Swan（1956）的工作标志这新古典增长理论的诞生。
Cass（1965）和Koopmans（1965）对新古典理论进行了扩展，引入家庭偏好，建立了最优增长的确定性理论。因此，
...
合并两个一阶条件可得：
$$
\frac{u^{\prime}\left(c_t\right)}{\beta u^{\prime}\left(c_{t+1}\right)}=1+f^{\prime}\left(k_{t+1}\right)-\delta
$$
...
基于所得到的值函数导数$v^{\prime}\left(k\right)$公式\eqref{bs}，自然可知：
$$
