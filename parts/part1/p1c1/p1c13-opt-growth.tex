...
% 1.3.1 ===== 有限期情形 =====
\subsection{最优增长模型求解：有限期情形} \label{subsec-rck-finite}
...
% 1.3.2 ===== 无限期情形 =====
\subsection{最优增长模型：无限期情形} \label{subsec-rck-infinite}
...
\begin{example}
    ...
    \end{equation*}
    由于资本序列收敛，并且$\lim\limits_{t\rightarrow \infty}\beta^t \alpha / (1-\alpha\beta)=0$，因此TVC条件成立，即
    \begin{equation*}
...
此外，求解无限期模型解析解的另一个方法是\textbf{猜测验证法}（Guess and Verify）。
但是，有解析解的例子在宏观模型中十分少见，因此我们在下一章将研究动态规划和数值方法。
