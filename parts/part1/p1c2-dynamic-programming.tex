...
\chapter{递归方法与动态规划} \label{ch-ngm-dp}

第一章提出了处理动态优化问题的两个视角：序列视角和递归视角，并对序列问题的求解进行了介绍。
本章则从递归的视角对动态演化问题进行进一步研究。
我们将继续借助最优增长模型，通过将原始的序列问题转化为递归形式，建立贝尔曼方程，进而引入确定性动态规划的基本理论方法；
...
\begin{lemma}   \label{lemma-ss-stable}
    稳态$k^*$满足\ref{eq-ss-rho}式和最优政策函数$g$满足$k<g(k)$，当且仅当$k<k^*$。
\end{lemma}
\begin{proof}
    由值函数性质可知：
...
\end{proof}
现在，考虑$k_0\in (0, k^*)$，根据引理\ref{lemma-ss-stable}可知，由$k_{t+1}=g(k_t)$生成的资本路径递增且有上界$k^*$。
