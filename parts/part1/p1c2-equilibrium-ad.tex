...
其次，研究这两种市场结构下竞争均衡与计划者问题（帕累托问题）均衡的关系，引出完备市场的概念，并证明代表性家庭的存在性；
再次，引用完备市场理论讨论资产定价问题；
...
完备市场是宏观经济学中的重要概念，它是指：经济中存在足够多的金融工具（如状态依存债券），等于未来所有可能的状态数，
个体通过交易这些工具构建资产组合，进而能够实现任意可行配置，对冲所有风险。
\begin{definition}[状态依存合约（State-Contingent Claims）]
    合约的支付依赖于未来某个事件（状态）$s^t$是否实现。例如如果未来可能发生$\{\text{晴天，下雨}\}$两种状态，消费者可以购买下雨时获得1单位消费品
    和晴天时获得1单位消费品的合约。
\end{definition}

通过如下两种经典的市场结构，可以实现市场的完备性，我们在此分别介绍。

（1）\textbf{阿罗-德布鲁市场}（Arrow-Debreu Market） 

在Arrow-Debreu结构下，市场只在$t=0$时刻开放，消费者在市场上交易的是对所有未来时刻$t\geq 1$的消费品的索取权（Claims）。
...
序贯市场也被称为\textbf{阿罗市场}（Arrow Market）或Radner市场，其中的单期状态依存合约被称为\textbf{阿罗债券}（Arrow Securities）。

同样，如果存在足够多的阿罗债券，序贯市场也可以达到完备。但是，如果交易工具不足，则序贯交易可能无法达成完备性。
例如我们在后面不完备市场理论中将会看到的，若经济中仅存在单期无风险债券，则市场是不完备的。

图\ref{gr-sqmarket-tree}给出了两状态马尔可夫链在序贯市场下，截至$t=2$时刻实现的一个特定历史，和给定这一实现的历史下，$t=3$时刻的两种可能性。
...
两种市场结构对比而言，阿罗-德布鲁市场更近似一个理论基准，而序贯交易更贴近于现实市场。
此外，对于上述两种市场结构，我们很自然的会想到：
\begin{itemize}
    \item 两种市场中的均衡的配置相同吗，与计划者均衡配置有什么关系？
...
同样是只依赖于所实现的总禀赋的。至此，我们就得到了计划者在参数$\{\lambda_i\}_{i=1}^I$下的一个帕累托有效配置，且均衡解的形式为：
\begin{equation*}
...
\begin{property}
    ...
    \begin{itemize}
        \item 历史关的（\textit{history-independent or history-free}：当前的$Y_t$是所有历史的充分统计量）；
        \item 与$t$时刻个体禀赋的分布无关（\textit{Distribution-free}：只有求和后的$Y_t$重要）；
        \item 也就是说，这组配置是经济所实现的总禀赋的时不变函数（$f$ \textit{time-invariant}）。
    \end{itemize}
\end{property}
...
阿罗德布鲁均衡时竞争均衡在完备市场下的实现形式。

回顾我们在本章开头提出的问题，计划者问题中的帕累托均衡是否能通过\textbf{市场配置}或者说\textbf{价格机制}实现？
我们之前已经介绍了阿罗-德布鲁市场和序贯市场两种市场结构，本节先研究前者，
我们将会证明：帕累托最优可以由Arrow-Debrue市场下的一组竞争均衡（Competitive Equilibrium）实现。

交易在$t=0$时发生，$s_0$已经实现。消费者只能在$t=0$时刻购买或售出未来$t$时刻消费品的索取权，这些权利依历史$s^t$而定（contingent on history $s^t$）。
又被称为阿罗-德布鲁债券（Arrow-Debreu Security）。
记$q_t^0\left(s_t\right)$为$t=0$时交易的阿罗-德布鲁债券的价格：他表示约定了在时间$t$和历史$s^t$实现时交割1单位消费品的阿罗-德布鲁债券的价格，
因此被称为阿罗-德布鲁价格（Arrow-Debreu Price）。
个体$i$\textbf{将价格视为给定}，面临预算约束：
...
由\ref{eq-ad-ci}式我们就能解得其余所有个体的消费配置。均衡消费具有形式：
\begin{equation*}
    c_t^i=g\left(\mu_i, Y_t ; \boldsymbol{\mu}\right)
\end{equation*}

\begin{property}
    阿罗-德布鲁市场中的竞争均衡具有如下特征：
    \begin{itemize}
        \item 历史无关，只依赖当前实现的总禀赋；
        \item 与$t$时刻个体禀赋的分布无关；
        \item $g$是时不变的。
    \end{itemize}
\end{property}

但需要注意的是，对于阿罗-德布鲁市场，我们此时\textbf{尚未得到一般均衡问题的解}。
在计划者的帕累托问题中，我们通过比值消去了乘子$\theta_t\left(s^t\right)$，其解$c_t^i\left(s^t\right)$只与总禀赋和权重比值有关，
而对计划者而言，权重是一组外生参数，因此解相对容易。
...
% 2.1.4.1
\subsubsection{竞争均衡价格}

一个自然的想法是通过每个个体的预算约束\ref{eq-ad-bc}来求解价格体系或乘子向量，因为截至目前我们仍未使用这一条件。
...
\end{equation*}
不难看出，可以构建出关于$I$个未知量$\mu_i$的$I$个非线性方程。由\ref{eq-ad-ci}可知，$\mu_i$以$\mu_i/\mu_1$的形式进入函数$g(\cdot)$，
因此我们可以正则化后进行求解。并且由\ref{eq-ad-c1}式，乘子$\mu_i$是依赖于财富分布的，二者反相关。

事实上，也可以从下面的视角进行理解：假设$\{c^i\}_{i=1}^{I}$是一个均衡配置，则一阶条件\ref{eq-ad-foc}式：
\begin{equation*} 
    \beta^t u_i^{\prime}\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right)=\mu_i q_t^0\left(s^t\right), \quad \text { for all } t, s^t
\end{equation*}
可以看作将价格体系$q_t^0\left(s^t\right)$描述为任意个体$i$的均衡配置的函数。因此，我们需要一个计算方法，使得可以先避开价格的计算就得到均衡配置。
我们下面将看到，通过解计划者问题将会提供极大的方便。
此外，由于价格体系的单位是任意的，因此其中一个价格可以被正则化，我们通常选择
...
我们还未回答的一个重要问题是：阿罗-德布鲁市场中的竞争均衡和社会计划者的帕累托最优均衡有什么关系。
...
\begin{equation*}
    \begin{aligned}
        ...
        \sum_i u^{\prime-1}\left(\frac{\mu_i}{\mu_1} u^{\prime}\left(c_t^1\left(s^t\right)\right)\right)
            &=\sum_i y_t^i\left(s^t\right)\equiv Y_t\left(s^t\right)
    \end{aligned}
\end{equation*}
对帕累托问题，权重$\boldsymbol{\lambda}$是一组外生参数，当其变化时，帕累托最优配置就可以构成一个前沿集合，也就是说帕累托均衡不止一个。
对比两个问题的最优条件发现，\textbf{竞争均衡配置就是一个特殊的帕累托最优配置}，其满足$\lambda_i = \mu_i^{-1}$。
同时，由于乘子$\mu_i$与个体$i$的财富反相关，这说明：竞争均衡意味着计划者给予了穷人更低的权重。

此外，在竞争均衡时，对应计划者问题的资源约束（注意不是预算约束）的影子价格$\theta_t\left(s^t\right)$等于竞争均衡价格$q_t^0\left(s^t\right)$。


% 2.1.4.3 
\subsubsection{福利经济学定理}

通过对比计划者问题的一阶条件\ref{eq-pareto-foc}：
\begin{equation*}
    \lambda_i \beta^t u_i^{\prime}\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right)=\theta_t\left(s^t\right), \quad \text { for all } i, t, s^t
\end{equation*}
...
\begin{equation*}
    \beta^t u_i^{\prime}\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right)=\mu_i q_t^0\left(s^t\right), \quad \text { for all } t, s^t
\end{equation*}
可知，当竞争均衡是帕累托最优，即$\lambda_i = \mu_i^{-1}$时，
应计划者问题的资源约束（注意不是预算约束）的影子价格$\theta_t\left(s^t\right)$等于竞争均衡价格$q_t^0\left(s^t\right)$。

% 2.1.4.4
\subsubsection{竞争均衡的算法}

我们已经得到，想要求解竞争均衡（数量-价格），必须要得到\ref{eq-ad-ci}和\ref{eq-ad-c1}式中的比值$\mu_i/\mu_1, \quad i=1,\cdots,I$。
下面的\textit{Negishi}算法可以完成这项工作：
\begin{itemize}
    \item 任意选取一个$\mu_i$，例如$\mu_1$，使其始终取一个固定的正数（正则化）。猜测其余$\mu_i$的取值，然后从过\ref{eq-ad-ci}和\ref{eq-ad-c1}式
        解得一个候选的配置$c^i, i=1,\cdots,I$
    \item 对任意一个个体$i$，利用一阶条件\ref{eq-ad-foc}求得价格体系$q_t^0\left(s^t\right)$
