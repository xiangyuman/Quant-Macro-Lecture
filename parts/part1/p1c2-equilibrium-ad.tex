...
\end{equation}
这意味着任意个体的消费都可以写为关于个体1的消费$c_t^1\left(s^t\right)$的函数。进一步，将上式代入资源约束\ref{eq-pareto-rc}可得：
...
进一步观察式\ref{eq-pareto-c1}可知，给定一组参数$\{\lambda_i\}_{i=1}^I$，$c_t^1\left(s^t\right)$\textbf{只依赖当前实现的总禀赋}
...
我们之前已经介绍了阿罗-德布鲁市场和序贯市场两种市场结构，在本节，
...
需要强调，个体$i$面临的预算约束是\textbf{单一}（single）的一个方程。原因在于：在阿罗-德布鲁市场中，所有交易都生在零时刻。
个体$i$的目标是选择$c^i\equiv \{c_t^i\left(s^t\right)\}_{t=0}^{\infty}$来最大化效用函数\ref{eq-cm-preference-i}：
\begin{equation*}
