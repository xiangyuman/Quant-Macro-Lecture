...
\textbf{完备市场}是宏观经济学中的重要概念，它与\textbf{不完备市场}的主要区别在于：经济中存在足够多的如状态依存合约等金融工具（数量覆盖未来所有可能的状态数），
...
\begin{definition}  \label{def-perfect-c-insurance}
    \textbf{完美消费保险}（或完美风险承担）是指一组消费配置$\{c^i=\{c_t^i\left(s^t\right)\}_{t=0}^{\infty}\}_{i=1}^{I}$：
...
在这样的市场上，家庭、企业等经济主体将价格视为给定，最优化其目标函数，均衡时所有市场出清。\textbf{阿罗-德布鲁均衡正是竞争均衡在一个完备市场下的实现形式}。
\begin{definition}
...
前面提到，阿罗-德布鲁市场天然是完备市场，这一市场有什么特点？能够实现完美风险承担？
...
\begin{equation}
    \beta^{t-\tau}\frac{\pi\left(s^t\right) u^{\prime}\left(c^i\left(s^t\right)\right)}{\pi\left(s^\tau\right)u^{\prime}\left(c^i\left(s^\tau\right)\right)}
        =\frac{q^0\left(s^t\right)}{q^0\left(s^\tau\right)} \quad \forall i, s^t, s^\tau
\end{equation}
即个体$i$通过使概率和价格调整后的$MRS=MRT$，实现\textbf{消费平滑}。

实际上，正如后面我们将证明的：阿罗-德布鲁市场（完备市场）是一组特殊权重下的帕累托均衡。因此必然满足完美消费保险。
\begin{property}
    阿罗-德布鲁均衡实现了完美消费保险。
\end{property}
...
% 2.1.4.1
\subsubsection{AD竞争均衡价格}    \label{subsubsec-ad-price}
...
\begin{equation*} 
    \beta^t u_i^{\prime}\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right)=\mu_i q_t^0\left(s^t\right) \quad \forall t, s^t
\end{equation*}
...
% 2.1.4.2
\subsubsection{AD竞争均衡的算法}  \label{subsubsec-ad-compu}
...
本小节研究另一种市场结构——序贯市场中的均衡（Sequential Market Equiliibrium, SME）。
虽然阿罗-德布鲁市场是具有良好性质的天然完备市场，但更适于作为一个理论基础，而序贯市场则更贴近于实践中的金融交易。
实际上，在序贯市场中，只要每期阿罗证券（单期状态依存证券）的数量足够覆盖下期所有状态（Complete Set of one-period-ahead Contingent Claims），
那么序贯市场也可以达到\textbf{动态完备（Sequentially Complete）}。
由此，我们自然会猜想：序贯市场（竞争）均衡应该也是帕累托最优的，并且与阿罗-德布鲁市场（竞争）均衡等价。我们将对此进行证明，并给出均衡存在的条件。

回顾序贯市场交易的特征：市场每期都会重新开放，但是只有一种单期状态依存合约，交易按期进行形成一个序列。
记在第$t$期历史$s^t$下签订的在第$t+1$期状态$s_{t+1}$实现时交割$1$单位消费品的\textbf{阿罗证券的价格}为$q_t\left(s^t,s_{t+1}\right)$；
记个体$i$购买（售出）的这一证券的\textbf{数量}为$a_{t+1}^i\left(s^t,s_{t+1}\right)$，从而其\textbf{预算约束}为：
\begin{equation}    \label{eq-sq-bc}
    c_t^i\left(s^t\right)+\sum_{s_{t+1}} q_t\left(s^t, s_{t+1}\right) a_{t+1}^i\left(s^t, s_{t+1}\right) 
        \leq y_t^i\left(s^t\right)+a_t^i\left(s^{t-1}, s_t\right)
\end{equation}
对$a_{t+1}^i\left(s^t,s_{t+1}\right)>0$，个体将会在$s_{t+1}$实现时收到消费品，因此是一种资产；反之是一种负债。

需要注意的是这一问题中个体$i$的\textbf{状态变量}：
在第$t$期，个体需要构建一个\textbf{资产组合}$\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}$，
即购买所有可能发生的状态（事件）$s_{t+1}\in S$对应的阿罗证券。一旦此后某一特定状态$s_{t+1}$实现，
只有对应于这一特定实现的$s_{t+1}$的证券$a_{t+1}^i\left(s^{t+1}\right)$才会支付（注意此时$t+1$的历史$s^{t+1}$形成），
进而决定个体$i$的下期开始时的资产状态（\textbf{Asset Position}）。
因此对第$t$期，不等式右侧$a_{t}^i\left(s^{t-1}, s_t\right)$可写为$a_t^i\left(s^t\right)$，
即在第$t$期决策时，个体状态变量（资产）由已经实现的历史$s^t$确定。

在竞争市场上，个体$i$将价格视为给定，其\textbf{序列问题}为：在预算约束\ref{eq-sq-bc}下，
通过选择\textbf{当期消费}$c_t^i\left(s^t\right)$和\textbf{资产组合}$\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}$最大化效用：
\begin{equation} \label{eq-sq-preference-i}
    U\left(c^i\right)=\sum_{t=0}^{\infty} \sum_{s^t} \beta^t u_i\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right)
\end{equation}  
记个体$i$第$t$期预算约束相关的乘子为$\mu_t^i\left(s^t\right)$，容易写出个体$i$的拉格朗日函数：
\begin{equation*}
    \begin{aligned}
        \mathcal{L}^i & =\sum_{t=0}^{\infty} \sum_{s^t} \beta^t u_i\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right) \\
        &+\sum_{t=0}^{\infty} \sum_{s^t} \mu_t^i\left(s^t\right)
            \left[y_t^i\left(s^t\right)+a_t^i\left(s^t\right)-c_t^i\left(s^t\right)
                -\sum_{s_{t+1}} q_t\left(s^t, s_{t+1}\right) a_{t+1}^i\left(s^t, s_{t+1}\right)\right]
        \end{aligned}
\end{equation*}

这里需要强调的有两点：
一是，在阿罗-德布鲁问题中，个体的预算约束是单一的一个方程；但在序贯市场中，个体面临的是序列问题，
因此约束条件是一系列不同期的预算约束，也就是说\textbf{乘子是时变的}。
二是，如上文刚强调过的，对任意第$t$期，个体构建的是所有可能发生状态（事件）$s_{t+1}\in S$
所对应的阿罗证券的资产组合$\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}$，它是$t+1$期消费品索取权的向量，
每一个元素都依赖于$t+1$可能实现的一个特定的状态（事件）$s_{t+1}$。
进一步求解优化问题，易得关于$c_t^i\left(s^t\right)$和
$\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}$中某一代表元素（如特定$s_{t+1}$）实现的一阶条件为：
\begin{align}
    \beta^t u_i^{\prime}\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right) = \mu_t^i\left(s^t\right) \\
    q_t\left(s^t, s_{t+1}\right) \mu_t^i\left(s^t\right) = \mu_{t+1}^i\left(s^t, s_{t+1}\right)
\end{align}
消去乘子可得：
\begin{equation}    \label{eq-sq-foc}
    q_t\left(s^t, s_{t+1}\right) = \beta \frac{u_i^{\prime}\left(c_{t+1}^i\left(s^{t+1}\right)\right)}{u_i^{\prime}\left(c_t^i\left(s^t\right)\right)} 
        \pi_{t}\left(s^{t+1} \mid s^t\right), \quad s^{t+1}=(s^t,s_{t+1}) \quad \forall s_{t+1}, t, s^t
\end{equation}


% 2.1.5.1 自然债务限制与
\subsubsection{自然债务限制}

回忆第一章介绍的无限期最优增长模型：经济需要横截性条件来排除资本存量无限增长。
对于定义序贯市场均衡，目前遗留了一个同样的问题：庞氏骗局（Ponzi Game），即个体不断滚动新债来偿还旧债和利息，导致消费者预算集无界，使得问题无解。
因此在定义序贯市场均衡时，我们需要一个横截性条件来排除庞氏骗局的可能。

能够排除庞氏骗局的条件是多样的。我们将看到，通过直接对债务水平施加一定约束，可以隐性的包含横截性条件，排除庞氏骗局。
通常我们会寻找一个最弱的债务限制，被称为\textbf{自然债务限制（Natural Debt Limit）}。
其想法是：\textbf{借款者的债务不能超过其未来收入的现值}，进而直接确保了还款能力，满足横截性条件（排除庞氏骗局）。
但是，对于债务额外施加约束条件，会导致最优化问题拉格朗日函数的变化。
我们将看到，自然债务约束在序贯均衡时是非紧的。因此，此前得到的一阶条件仍然是正确的，进而能为研究序贯市场均衡与阿罗-德布鲁均衡的关系提供方便。

为了方便理解\textbf{自然债务限制}，首先看一个简单的局部均衡例子。
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
排除庞氏骗局要求无穷远处资产的现值大于等于0（实际上必然取等号，不再证明），即有如下横截性条件：
\begin{equation*}
...
\end{equation*}
又由于消费的非负性，因此可得\textbf{自然债务限制}：
\begin{equation*}
    a_t \geq -\sum_{j=1}^{\infty}\frac{y_{t+j}}{\left(1+r\right)^{j+1}}
\end{equation*}

需要指出的是，自然债务限制与横截性条件虽然直觉上相似，并且都能够排除庞氏骗局，但并不等同。
个体每一期都面临自然债务限制（约束序列），而横截性条件是一个长期极限条件。

现在回到序贯市场的讨论中。在序贯均衡时，问题相对变得复杂。我们首先需要理解尾部资产的相关概念。



% Kruger
对于给定的一组阿罗债券的价格过程$q=\{q_t\left(s^t,s_{t+1}\right)\}$，定义零时刻状态依存的消费品索取权价格





\begin{equation}    \label{eq-sq-natural-debt-lim}
    \tilde{a}_{t+1}^i\left(s^{t+1}\right) \geq -A_{t+1}^i\left(s^{t+1}\right)
\end{equation}

记$\nu_t^i\left(s^t;s_{t+1}\right)$为个体$i$第$t$期面临的、在$t+1$期实现状态$s_{t+1}$时的自然债务限制约束乘子，则正确的拉格朗日函数为：
\begin{equation*}
    \begin{aligned}
        \mathcal{L}^i & =\sum_{t=0}^{\infty} \sum_{s^t} \beta^t u_i\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right) \\
        &+\sum_{t=0}^{\infty} \sum_{s^t} \mu_t^i\left(s^t\right)
        ...
        &+ \sum_{t=0}^{\infty} \sum_{s^t} \sum_{s_{t+1}} \nu_t^i\left(s^t,s_{t+1}\right)
            \left[A_{t+1}^i\left(s^{t+1}\right)+a_{t+1}^i\left(s^{t+1}\right)\right]
        \end{aligned}
\end{equation*}
则正确的一阶条件为：
\begin{align}
...
但是，\textbf{均衡时自然债务限制约束一定是非紧的}：假设存在某一历史$s^{t+1}$，带来一个紧的自然债务限制，那么，
消费者从此时开始，必须设定消费为0，才能保证偿还债务。也就是说，如果自然债务限制为紧，则个体未来的收入全部被用于偿还债务。
但根据效用函数的Inada条件，$\lim\limits_{c\rightarrow 0}u_i\left(c\right)=+\infty$，这意味着，未来的边际效用将会是无穷。
那么，个体只需要将此前的部分消费推迟到自然债务限制为紧之后，就能找到一个可行的更好的消费配置。
因此，乘子$\nu_t^i\left(s^t;s_{t+1}\right)$始终为0，之前得到的最优条件\ref{eq-sq-foc}是正确的，即：
\begin{equation}    \label{eq-sq-opt}
    q_t\left(s^t, s_{t+1}\right) = \beta \frac{u_i^{\prime}\left(c_{t+1}^i\left(s^{t+1}\right)\right)}{u_i^{\prime}\left(c_t^i\left(s^t\right)\right)} 
        \pi_{t}\left(s^{t+1} \mid s^t\right), \quad s^{t+1}=(s^t,s_{t+1}) \quad \forall s_{t+1}, t, s^t
\end{equation}
...
\begin{definition}
    \textbf{序贯市场均衡}（Sequential Market Equilibrium，SME），
    由满足如下条件的\textbf{阿罗证券的价格}$\{q\left(s^t,s_{t+1}\right)\}_{t=0, s_{t+1}\in S}^{\infty}$和一组配置
    $\left\{ \left[c_t^i\left(s^t\right), \{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S} \right]_{t=0}^{\infty} \right\}_{i\in I}$
    构成的一个\textbf{竞争均衡}：
    \begin{itemize}
        \item 给定价格，每个个体$i$选择的配置
            $\left\{ \left[c_t^i\left(s^t\right), \{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S} \right]_{t=0}^{\infty} \right\}$
            在预算约束\ref{eq-sq-bc}下最大化期目标\ref{eq-sq-preference-i}
        \item 产品和金融市场出清：
            \begin{align*}
                \sum_{i=1}^I c_t^i\left(s^t\right)&=\sum_{i=1}^I y_t^i\left(s^t\right) \\
                \sum_{i=1}^I a_{t+1}^i\left(s^t, s_{t+1}\right)&=0 \quad \forall s_{t+1} \in S
            \end{align*}
    \end{itemize}
\end{definition}

回忆在\textbf{阿罗-德布鲁均衡}下，在第$0$期交易的第$t$期消费品的状态依存合约的价格可以写为：
\begin{equation*}
    q_t^0\left(s^t\right)=\beta^t \frac{u_i^{\prime}\left(c_t^i\left(s^t\right)\right)}{\mu_i}\pi_t(s^t)
\end{equation*}
其中乘子$\mu_i$是个体$i$的终身预算约束。对比序贯市场均衡的最优条件，不难看出阿罗证券的价格可以写为：
\begin{equation}    \label{eq-sq-p-ad}
    q_t\left(s^t,s_{t+1}\right) = \frac{q_{t+1}^0\left(s^{t+1}\right)}{q_t^0\left(s^t\right)}
\end{equation}
事实上，两个市场间均衡的冠以可以由如下定理进一步概括：
\begin{theorem}
    若
\end{theorem}
\begin{proof}
...
事实上，在一些宏观模型中，均衡可能并非帕累托最优，或是难以通过序列形式求解，此时定义递归均衡（Recursive Comptetitive Equilibrium）是非常有效的。
...
% 2.4 =============== Kruger Decentralized Equilibrium Lafo ===============
