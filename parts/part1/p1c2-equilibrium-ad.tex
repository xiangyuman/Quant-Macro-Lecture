...
\begin{example}
    ...
    式\ref{eq-price-tail}有非常多运用，例如在$\tau$时刻购买的许诺从此刻$\tau$开始分红的股权定价。
...
和\textbf{资产组合的序列}$\{\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}\}_{t=0}^{\infty}$最大化效用：
...
实际上，\textbf{均衡时自然债务限制约束一定是非紧的}：假设存在某一历史$s^{t+1}$，带来一个紧的自然债务限制，那么，
消费者从此时开始，必须设定今后消费为0，才能保证偿还债务。也就是说，如果自然债务限制为紧，则个体未来的收入全部被用于偿还债务。
但效用函数的Inada条件，$\lim\limits_{c\rightarrow 0}u_i\left(c\right)=+\infty$意味着：未来的边际效用将会是无穷。
...
\begin{definition}
    \textbf{财富分布}（Distribution of Wealth），是一组向量
        $\vec{a}_t\left(s^t\right)=\left\{a_t^i\left(s^t\right)\right\}_{i=1}^I$，
        满足：$\sum_{i}a_t^i\left(s^t\right)=0$。
\end{definition}

我们将在后文证明两个市场均衡等价性时详细讨论关于财富的问题。现在给出是因为它是定义均衡的一个必要条件。

\begin{definition}
    \textbf{序贯市场均衡}（Sequential Market Equilibrium）是一个竞争均衡，
    由初始财富分布$\vec{a}_0\left(s_0\right)=\{a_0^i\left(s_0\right)\}_{i\in I}$、
    ...
    和\textbf{阿罗价格}$\{q\left(s^t,s_{t+1}\right)\}_{t=0, s_{t+1}\in S}^{\infty}$和
    构成，满足：
    \begin{itemize}
...
结合两个竞争市场的均衡配置，我们很容易发现二者的均衡是等价的。我们的证明思路是：给定A市场下的价格-消费配置，
证明在B市场存在一个均衡价格，使得给定的消费和B市场中的这一价格符合B市场的均衡条件。
...
\begin{equation*}
    q_t\left(s^t, s_{t+1}\right) = \beta \frac{u_i^{\prime}\left(\tilde{c}_{t+1}^i\left(s^{t+1}\right)\right)}
        {u_i^{\prime}\left(\tilde{c}_t^i\left(s^t\right)\right)} 
            \pi_{t+1}\left(s^{t+1} \mid s^t\right), \quad s^{t+1}=(s^t,s_{t+1}) \quad \forall s_{t+1}, t, s^t
\end{equation*}

现在，\textbf{给定$\left(\{c_t^i\left(s^t\right)\},q_t^0\right)$为阿罗-德布鲁市场中的一个竞争均衡配置}。
...
\end{equation}
从而对我们猜测（构造）的价格$q_t\left(s^t, s_{t+1}\right)$，有：
\begin{equation}
    q_t\left(s^t, s_{t+1}\right) = \frac{q_{t+1}^0\left(s^{t+1}\right)}{q_t^0\left(s^t\right)}
...
也就是说，\textbf{阿罗-德布鲁市场中给定的这组竞争均衡配置使得序贯市场的均衡条件成立}。反之同理。

\begin{theorem}
    若$\left(c,q\right)$是一个阿罗-德布鲁均衡，那么$(a,c,\tilde{Q})$是一个序贯市场均衡，其价格满足：
    $$\tilde{Q}_t\left(s^t,s_{t+1}\right) = \frac{q_{t+1}^0\left(s^{t+1}\right)}{q_t^0\left(s^t\right)}$$
    若$(a,c,\tilde{Q})$是一个序贯市场均衡，那么$\left(c,q\right)$是一个阿罗-德布鲁均衡，其价格满足：
    \begin{equation}
        \begin{aligned}
...
本节开头的论证似乎已经完成了对于这一定理的证明，但我们还遗漏了一个问题，即序贯市场中的财富有什么作用？对于这一问题的讨论有些复杂。

首先，我们需要为个体在$t$时刻历史$s^t$下的值函数找到一个合适的\textbf{状态变量}。我们借助阿罗-德布鲁均衡来构造。
思考如下问题：对于所有交易都在$t=0$发生的竞争均衡，个体$i$在$t$时刻历史$s^t$实现后的\textbf{延续财富}（Continuation Wealth）是多少？
