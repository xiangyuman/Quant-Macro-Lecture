...
\begin{definition}
    \textbf{价格系统}（Price System）由一组函数序列$\{q_t^0\left(s^t\right)\}_{t=0}^{\infty}$构成；
...
\end{equation*}
均衡时$\mu_i$是依赖于个体禀赋分布的$y_t^i\left(s^t\right)$的，因此不同个体禀赋分布会带来不同配置。
换句话说，如果我们想得到某一配置结果$c^i=\{c_t^i\left(s^t\right)\}_{t=0}^{\infty}$，我们可以通过改变$y^i$，
找到一个能够带来这一配置的乘子$\mu_i$。简言之，\textbf{任意可行均衡配置都能通过适当重新分配财富来实现}。
...
可以乘上一个常数而不会影响结果，即可以正则化后进行求解。例如固定某一乘子$\mu_1$，调整其他乘子。实际上，\textbf{正则化相当于调整价格系统的计价单位}。
...
我们已经知道：想要求解竞争均衡（数量-价格），必须想办法得到\ref{eq-ad-ci}和\ref{eq-ad-c1}式中的比值$\mu_i/\mu_1, \quad i=1,\cdots,I$。
...
\begin{itemize}
    ...
    \item 福利经济学第二定理表明：任意帕累托均衡都可以由某一竞争均衡支撑。前文指出，由于乘子$\mu_i$依赖于财富分布，
        我们可以通过重新分配财富，找到一个与目标配置相匹配的乘子。也就是说，\textbf{任意可行均衡配置都能通过适当重新分配财富来实现}。
        因此福利经济学第二定理成立。
\end{itemize}
