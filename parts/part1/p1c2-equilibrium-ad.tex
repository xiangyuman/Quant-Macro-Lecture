...
我们之前提到，虽然阿罗-德布鲁市场是具有良好性质的天然完备市场，但更适于作为一个理论基础，而序贯市场则更贴近于实践中的金融交易。
...
由此，我们自然会猜想：序贯市场（竞争）均衡应该也是帕累托最优的，并且与阿罗-德布鲁市场（竞争）均衡等价。我们将对此进行证明，并给出均衡的存在条件。

回顾序贯市场交易的特征：市场每期都会重新开放，但是只有一种单期状态依存合约，交易按期进行形成一个序列。
记在第$t$期历史$s^t$下签订的在第$t+1$期状态$s_{t+1}$实现下交割$1$单位消费品的\textbf{阿罗证券的价格}为$q_t\left(s^t,s_{t+1}\right)$；
...
\end{equation}
对$a_{t+1}^i\left(s^t,s_{t+1}\right)>0$，个体将会在$s_{t+1}$实现时收到消费品，因此是一种资产；
反之是一种负债。
需要注意的是这一问题中个体$i$的\textbf{状态变量}：
在第$t$期，个体会购买所有可能发生的状态（事件）$s_{t+1}\in S$对应的阿罗证券，
即在第$t$期需要构建一个\textbf{资产组合}$\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}$。一旦此后某一特定状态$s_{t+1}$实现，
只有对应于这一特定实现的$s_{t+1}$的债券$a_{t+1}^i\left(s^{t+1}\right)$才会支付（注意此时$t+1$的历史$s^{t+1}$形成），
...
\begin{equation}
    U\left(c^i\right)=\sum_{t=0}^{\infty} \sum_{s^t} \beta^t u\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right)
\end{equation}  
记个体$i$第$t$期预算约束相关的乘子为$\mu_t^i\left(s^t\right)$，容易写出个体$i$的拉格朗日函数：
\begin{equation*}
    \begin{aligned}
        \mathcal{L}^i & =\sum_{t=0}^{\infty} \sum_{s^t} \beta^t u\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right) \\
...
因此约束条件是一系列不同期的预算约束，也就是说乘子是时变的。
二是，如上文刚强调过的，对任意$t$期，个体构建的是所有可能发生状态（事件）$s_{t+1}\in S$
所对应的阿罗证券的资产组合$\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}$，它是$t+1$期消费品索取权的向量，
每一个元素都依赖于$t+1$可能实现的一个特定的状态（事件）$s_{t+1}$。
进一步求解优化问题，易得关于$c_t^i\left(s^t\right)$和$\{a_{t+1}^i\left(s^t,s_{t+1}\right)\}_{s_{t+1}\in S}$中代表元素（如特定$s_{t+1}$）实现的一阶条件为：
\begin{align}
    \beta^t u^{\prime}\left(c_t^i\left(s^t\right)\right) \pi_t\left(s^t\right) = \mu_t^i\left(s^t\right) \\
    q_t\left(s^t, s_{t+1}\right) \mu_t^i\left(s^t\right) = \mu_{t+1}^i\left(s^t, s_{t+1}\right)
\end{align}
消去乘子可得：
\begin{equation}    \label{eq-sq-foc}
    q_t\left(s^t, s_{t+1}\right) = \beta \frac{u^{\prime}\left(c_{t+1}^i\left(s^{t+1}\right)\right)}{u^{\prime}\left(c_t^i\left(s^t\right)\right)} 
...
至此我们已经完成了主要工作，但是对于定义序贯市场均衡，还遗留了一个重要问题：庞氏骗局（Ponzi Game）。
第一章介绍无限期最优增长模型时，我们提到，经济需要横截性条件来排除资本存量无限增长。
在定义序贯市场均衡时，我们也需要一个类似的条件来确保借款者未来的还款能力，排除庞氏骗局。
否则，无限期模型中的个体将会不断地借新还旧，导致消费者预算集无界，使得问题无解。

实践中，我们希望增加的这一债务约束在均衡时是非紧的，这样此前得到的一阶条件仍然是正确的，进而为研究序贯市场均衡与阿罗-德布鲁均衡的关系提供方便。
排除庞氏骗局的条件是多样的，但通常我们会寻找一个最弱的债务限制，被称为\textbf{自然债务限制（Natural Debt Limit）}，
其想法是：借款者的债务不能超过其未来收入的现值。我们主要研究这一限制。

首先看一个简单的局部均衡例子。假设一个无限期经济体中，禀赋为外生过程，代表性家庭每期的预算约束为：
\begin{equation*}
    c_t+a_{t+1} \leq y_t + \left(1+r\right)a_t
\end{equation*}
其中$a$为资产（小于零时为负债），$r$为恒定的利率（局部均衡）。
对$t=0$：
\begin{equation*}
    a_1 = y_0+(1+r)a_0-c_0
\end{equation*}
...
\begin{equation*}
    \begin{aligned}
        a_2 &= y_1+(1+r)a_1-c_1 \\
            &= y_1+(1+r)\left[y_0+\left(1+r\right)a_0-c_0\right]-c_1 \\
            &= y_1+(1+r)y_0+(1+r)^2a_0-(1+r)c_0-c_1
    \end{aligned}
\end{equation*}
依次递归展开，可得终身预算约束为：
\begin{equation*}
    \sum_{t=0}^{\infty}\frac{c_t}{\left(1+r\right)^t} \leq \sum_{t=0}^{\infty}\frac{t_t}{\left(1+r\right)^t} + a_0
\end{equation*}
为了防止无限制积累债务，要求未来财富的现值大于等于零，即：
\begin{equation*}
    \lim_{T\rightarrow\infty}\frac{a_T}{\left(1+r\right)^T} \geq 0
\end{equation*}
等价于：
\begin{equation*}
    a_t \geq -\sum_{s=t}^{\infty}\frac{y_s}{\left(1+r\right)^{s-t}}
\end{equation*}
在序贯市场均衡中，问题相对变得复杂。我们首先需要理解尾部资产的相关概念。

对于给定的一组阿罗债券的价格过程$q=\{q_t\left(s^t,s_{t+1}\right)\}$，定义零时刻状态依存的消费品索取权价格



需要额外指出的是，自然债务限制与横截性条件虽然直觉上相似，但并不是同一件事。在序贯市场中，个体每一期都面临自然债务限制，
而横截性条件是长期极限条件。
