% 2.2 =============== 序列问题到递归问题 ===============
\section{递归形式下的最优增长模型}  \label{sec-fe-rck}

本节，我们将会把无限期最优增长模型转化为递归形式，基于其贝尔曼方程，具体而深入地研究动态规划的相关理论方法。

% 2.2.1 ===== 值函数、状态变量与贝尔曼方程 =====
\subsection{最优增长模型的贝尔曼方程} \label{subsec-bellman-rck}

对于最优增长问题，唯一可变变量是$k_0$：给定不同的初始资本将会得到不同的最优化序列和最大化效用值。因此，可以定义最大化目标效用值的函数：
\begin{equation*}
    v\left(k_0\right) \equiv 
        \max _{\left\{k_{t+1}\right\}_{t=0}^{\infty}} 
        \sum_{t=0}^{\infty} \beta^t u\left(f\left(k_t\right)+\left(1-\delta\right)k_t-k_{t+1}\right), \quad 0<\beta<1
\end{equation*}
