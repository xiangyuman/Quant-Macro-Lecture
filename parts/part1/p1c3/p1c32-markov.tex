...
本节先对马尔可夫过程理论进行讲解。上一节曾指出，
处理连续动态规划的总体思路主要有离散化估计和平滑估计，例如VFI和Collocation，
但无论选取何种方法，\textbf{计算的主要难点}都在于高维的状态及期望（积分）的计算。
...
换句话说，马尔可夫近似虽然与数值积分不等同，但缺少这一性质，数值积分也很难进行。因此，马尔可夫假设对于宏观模型至关重要。
...
% 3.2.3 ===== Conditional Expectation and Discrete AR(1) =====
...
给定$k=k_m,~z=z_i$，对值函数$v\left(k_m,z_i\right)$，利用转移概率军阵可以直接写出如下贝尔曼方程：
\begin{equation*}
    \begin{aligned}
        v\left(k_m,z_i\right) &= \max_{k^{\prime}}u\left[zf(k_m)+(1-\delta)k_m-k^{\prime}\right] + 
            \mathbbm{E}\left[v\left(k^{\prime},z^{\prime}\right)\mid z=z_i\right] \\
        &= \max_{k^{\prime}}u\left[zf(k_m)+(1-\delta)k_m-k^{\prime}\right] +
            \boldsymbol{P}_i v\left(k^{\prime},\mathbbm{z}\right)
    \end{aligned}
\end{equation*}
其中$v\left(k^{\prime},\mathbbm{z}\right)$是一个$n$维列向量，每行对应下期$z^{\prime}$在空间$\mathbbm{z}$上可能的取值
$\left(z_1,\cdots,z_n\right)$。

我们可以将其写为更为紧凑的矩阵形式。记$v\left(k_m,\mathbbm{z}\right)$是$n$维值函数，每行对应TFP的一种可能取值下的最优值。
...
\begin{equation*}
    \begin{aligned}
        ...
        &= \max_{k^{\prime}}u\left[\mathbbm{z}f(k_m)+\vec{\boldsymbol{1}}(1-\delta)k_m-\vec{\boldsymbol{1}}k^{\prime}\right] + 
            \mathbbm{E}\left[v\left(k^{\prime},\boldsymbol{z}^{\prime}\right)\mid \mathbbm{z}\right] \\
        &= \max_{k^{\prime}}u\left[\mathbbm{z}f(k_m)+\vec{\boldsymbol{1}}(1-\delta)k_m-\vec{\boldsymbol{1}}k^{\prime}\right] +
            \boldsymbol{P} v\left(k^{\prime},\mathbbm{z}\right)
    \end{aligned}
\end{equation*}